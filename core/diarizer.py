...
class AudioTranscriber:
    ...
    def transcribe(
        ...
        if cached_transcribe:
            ...
            # 注入 initial_prompt：当前 whisperx 版本的 transcribe() 不接受该关键字参数，
            # 需写入 pipeline 的解码选项（TranscriptionOptions.initial_prompt）。
            # 每次都显式设置（含清空），避免缓存复用的模型残留上一次的 prompt。
            try:
                import dataclasses as _dc
                model.options = _dc.replace(
                    model.options, initial_prompt=(initial_prompt or None)
                )
                if initial_prompt:
                    _log(f"已将 initial_prompt 注入模型解码选项（{len(initial_prompt)} 字符）")
                    # 回读自检：确认 prompt 真的写进了 pipeline 解码选项
                    _readback = getattr(model.options, "initial_prompt", None)
                    if _readback:
                        _preview = _readback[:60].replace("\n", " ")
                        _log(f"回读 model.options.initial_prompt 成功（{len(_readback)} 字符）：{_preview}...")
                    else:
                        _log("回读 model.options.initial_prompt 为空，prompt 可能未写入！", "warning")
            except Exception as e:
                _log(f"注入 initial_prompt 失败，将忽略上下文提示继续转录: {e}", "warning")
