...
class Glossary:
    ...
    def from_dict(cls, data: dict) -> "Glossary":
        return cls(
            ...
            people=cls._normalize_people((data or {}).get("people") or []),
            terms=list((data or {}).get("terms") or []),
        )

    @staticmethod
    def _normalize_people(raw: list) -> list[dict]:
        """把 people 统一归一化为 [{"name": str, "alias": list[str]}, ...]。

        兼容历史/手工编辑的数据：元素可能是纯字符串（如 "李四"），
        也可能是缺字段的字典。统一转成标准字典，避免下游 p.get() 报错。
        """
        normalized: list[dict] = []
        for p in raw or []:
            if isinstance(p, dict):
                name = str(p.get("name", "") or "").strip()
                alias_raw = p.get("alias") or []
                alias = [str(a).strip() for a in alias_raw if str(a).strip()]
            ...
            if not name:
                continue
            normalized.append({"name": name, "alias": alias})
        return normalized
